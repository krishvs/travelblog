...
(function ($) {
  ...
  * @param {void}
  */
  console.log('going to define the function')
  $.fn.mediumInsert = function (options) {
    console.log('I am in all plugin');
...
(function ($) {
  ...
  $.fn.mediumInsert.registerAddon('embeds', {
    ...
    setEnterActionEvents : function () {
      ...
      console.log('The value of the embed tag is ',embed_tag);
      if (!embed_tag) {
  ...
  $.fn.mediumInsert.registerAddon('maps', {
    ...
    add : function ($placeholder) {
      ...
      var formHtml = '<div class="medium-editor-toolbar-form-anchor mediumInsert-mapsWire" style="display: block;"><input type="text" value="" placeholder="type or paste url here" class="mediumInsert-mapsText"></div>';
      $(formHtml).appendTo($placeholder.prev());
      setTimeout(function () {
        $placeholder.prev().find('input').focus();
      }, 50);
      ...
      this.currentPlaceholder = $placeholder;
      $(".mediumInsert-mapsText").focus();
    },


    setmapButtonEvents : function () {
      var that = this;
      $(document).on('keypress', 'input.mediumInsert-mapsText', function (e) {
        if ((e.which && e.which == 13) || (e.keyCode && e.keyCode == 13)) {
          that.setEnterActionEvents();
          that.removeToolbar();
        }
      ...
      this.$el.on('blur', '.mediumInsert-mapsText', function () {
        that.removeToolbar();
      });
    ...
    setEnterActionEvents : function () {
      ...
      var url = $("input.mediumInsert-mapsText").val();
      if (!url) {
        return false;
      }
      var map_tag = that.convertUrlTomapTag(url);
      console.log('The value of the map tag is ',map_tag);
      if (!map_tag) {
        alert('Incorrect URL format specified');
        return false;
      } else {
        map_tag = $('<div class="mediumInsert-maps"></div>').append(map_tag);
        that.currentPlaceholder.parent().append(map_tag);
      }
    },

    removeToolbar : function () {
      $(".mediumInsert-mapsWire").remove();
    },

    convertUrlTomapTag : function (url) {
      var map_tag = '<iframe width="420" height="315" src="'+url+'"></iframe>'
      return map_tag;
    }
